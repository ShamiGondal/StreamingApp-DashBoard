...
useEffect(()=>{
  ...
  if(date){
    toast({
      description: (<p>{JSON.stringify(date.toLocaleDateString() )} is added</p>),
    })
  }
},[date])
      ...
      <SelectContent>
        <SelectGroup>
         ...
         {sportId && leagueId && teamId1 && teamId2 ?<div className="flex flex-wrap gap-3 items-center justify-center">
            <Input type='text'placeholder='Match Venue' ref={venue} className=''></Input>
         <div className=''>  <CalendarForm setDate = {setDate} ></CalendarForm></div>
        ...
        <DialogFooter>
          <Button type="submit" disabled={ (venue==null) ||(!sportId) || (!leagueId) || (!teamId1) || (!teamId2)  || (!date) || (! (teamId1!==teamId2) || (!links.length))} onClick={handleClick} >{loading?"Loading":"Add Game"}</Button>
        </DialogFooter>
      </DialogContent>
